...
const EditPaymentModal = ({ OpenModal, setOpenModal, paymentData }) => {
  console.log(paymentData);
  const [userType, setUserType] = useState(paymentData.user_type || "");
  const [userId, setUserId] = useState(paymentData.user_Id || "");
  const [userName, setUserName] = useState(paymentData.user_name || "");
  const [depositor, setDepositor] = useState(paymentData.depositor || "");
  const [paymentType, setPaymentType] = useState(
    paymentData.payment_type || ""
  );
  const [bankName, setBankName] = useState(paymentData.bank_name || "");
  const [bankNumber, setBankNumber] = useState(paymentData.bank_number || "");
  const [amount, setAmount] = useState(paymentData.amount || "");
  const [date, setDate] = useState(
    moment(new Date(paymentData.date * 1000)).format("YYYY-MM-DD") || ""
  );
  const [desc, setDesc] = useState(paymentData.desc || "");
  ...
  return (
    <ModalWrapper
      ...
      title={"Edit Payment"}
    >
      <div className="flex flex-wrap justify-center gap-x-4 gap-y-4 py-4 overflow-y-auto max-h-[60vh]">
        <div className="flex gap-x-4 flex-wrap gap-y-4 justify-center ">
      ...
      <div className="w-[100%] flex justify-center my-5 mt-1">
        {Loading ? (
          ...
          <button
            ...
            className="w-[50%] hover:bg-[#394B92] py-3 hover:text-white border-2 border-[#394B92] text-[#394B92] font-[900] text-[.9rem] sm:text-sm md:text-[1rem] lg:text-xl hover:rounded-xl transition-all ease-in-out duration-500 whitespace-nowrap"
          >
            Update Payment
          </button>
        )}
