...
import { EditStockStatsApi, UpdateItemQtyApi } from "../../Https";
...
const EditStockModal = ({ OpenModal, setOpenModal, CurrentState }) => {
  console.log(CurrentState);
  const [CompanyId, setCompanyId] = useState("");
  const [ArticleId, setArticleId] = useState(CurrentState.articleId._id || "");
  const [ArticleName, setArticleName] = useState(
    CurrentState.articleId.name || ""
  );
  const [size, setSize] = useState(CurrentState.sizeId.size || "");
  const [sizeId, setSizeId] = useState(CurrentState.sizeId._id || "");
  const [NewStock, setNewStock] = useState(CurrentState.qty || "");
  const [Purchase, setPurchase] = useState(CurrentState.purchase || 0);
  const [InvoiceNo, setInvoiceNo] = useState(CurrentState.invoice_no || "");
  const [TruckNo, setTruckNo] = useState(CurrentState.truck_no || "");
  const [CurrentDate, setDate] = useState(
    moment(new Date(CurrentState.date * 1000)).format("YYYY-MM-DD") || ""
  );
  const [Desc, setDesc] = useState(CurrentState.desc || "");
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      try {
        const response = await EditStockStatsApi(
          Supplier
            ? {
                stockId: CurrentState._id,
                articleId: ArticleId,
  ...
  const [AccountType, setAccountType] = useState(
    CurrentState.type === 1 ? "Self" : CurrentState.type === 2 && "Supplier"
  );
  const [Supplier, setSupplier] = useState("");
  const [SearchTextPop, setSearchTextPop] = useState("");

  return (
    <ModalWrapper open={OpenModal} setOpen={setOpenModal} title={"Edit Stock"}>
