...
import { CompanyReportColumns } from "../../utils/ColumnsData/CompanyReportColumns";
...
const CompanyReport = () => {
  ...
  const totalAmount = useMemo(() => {
    return CompanyState.data.reduce((acc, cust) => acc + cust.total, 0);
  }, [CompanyState.data]);
  const openingBalance = useMemo(() => {
    return CompanyState.data.reduce(
      (acc, cust) => acc + cust.opening_balance,
      0
    );
  }, [CompanyState.data]);
  const recievedAmount = useMemo(() => {
    return CompanyState.data.reduce((acc, cust) => acc + cust.paid, 0);
  }, [CompanyState.data]);
  const recievealeAmount = useMemo(() => {
    return CompanyState.data.reduce((acc, cust) => acc + cust.remaining, 0);
  }, [CompanyState.data]);
